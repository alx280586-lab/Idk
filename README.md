...
A minimal byte-level dataset lives at `fnc/data/base_corpus.txt`. It contains a
few sentences that exercise the procedural generator and is the default target
for bootstrap runs. You can copy it elsewhere—or swap in your own text—using the
helper script:

```bash
./fnc/scripts/prepare_data.sh data/base_corpus.txt
```

Update `configs/base.yaml` (or the hardware-specific presets) if you want to
point at a custom dataset path. The new `configs/simple_base.yaml` preset already
targets the bundled corpus for quick experiments.
...
```bash
./fnc/scripts/train_small.sh --steps 50 --checkpoint-dir runs/stage1
```
